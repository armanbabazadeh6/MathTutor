...
export function StreakFlame({ count, lit = true }: { count: number; lit?: boolean }) {
  return (
    <div
      ...
      style={{ opacity: lit ? 1 : 0.6, boxShadow: "0 3px 0 var(--chunky-shadow)" }}
    >
      <svg
        viewBox="0 0 24 28"
        width="20"
        height="24"
        aria-hidden
        className={lit ? "mt-flicker" : undefined}
      >
        <path
      ...
      <span className="font-display text-kid-lg font-bold tabular-nums text-accentink">{count}</span>
    </div>
  );
}
